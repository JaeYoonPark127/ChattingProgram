...
public class EightServer extends JFrame implements Runnable {
	...
	EightServer() {
		...
		jp.setLayout(new GridLayout(2,3)); // 그리드 레이아웃
		
		JScrollPane jsp = new JScrollPane(); // 텍스트에어리어에 스크롤 추가
		...
		jta.setEditable(true); // 에디팅 불가
		jta.setLineWrap(true); // 자동줄바꿈
		jta.setText("Server Start...1\n");

		getContentPane().add(jp); // 프레임에 패널 붙임
		
		btnNewButton = new JButton("관리자 모드");
		jp.add(btnNewButton);
